...
def load_and_fill_wbmodel_focalloss_cfg(path_cfg, model_cfg_file):
  model_cfg = model.netvlad.ModelWBFocalLossCfg()
  model_cfg.load(model_cfg_file)
  data = np.load(path_cfg.init_weight_file)
  model_cfg.proto_cfg.centers = data['centers']
...
if __name__ == '__main__':
  ...
  if bool(opts.is_wb):
    if bool(opts.is_focal_loss):
      model_cfg = load_and_fill_wbmodel_focalloss_cfg(path_cfg, opts.model_cfg_file)
      _model = model.netvlad.NetVladWBFocalLossModel(model_cfg)
    else:
