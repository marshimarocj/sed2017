...
def sample_data_for_center():
  ...
  ft_root_dir = os.path.join(root_dir, 'twostream', 'feat_anet_flow_5frame')
  num_sample = 10000
...
# TODO
def sample_data_for_twostream_sync_center():
  root_dir = '/home/jiac/data/sed' # xiaojun
  flow_ft_root_dir = os.path.join(root_dir, 'twostream', 'feat_anet_flow_5frame')
  rgb_ft_root_dir = os.path.join(root_dir, 'vgg19_pool5_fullres')
  num_sample = 10000
  out_file = os.path.join(flow_ft_root_dir, 'sync.sample.%d.npy'%num_sample)
  ...
  for name in names:
    flow_ft_dir = os.path.join(flow_ft_root_dir, name)
    flow_ft_db = apid.db.VGG19FtDb(flow_ft_dir) # it's a hack

    rgb_ft_dir = os.path.join(rgb_ft_root_dir, name)
    rgb_ft_db = api.db.VGG19FtDb(rgb_ft_dir)

    print name

    for chunk in rgb_ft_db.chunks:
      print chunk
      rgb_fts = rgb_ft_db.load_chunk(chunk)
      flow_fts = flow_ft_db.load_chunk(chunk)
      shape = rgb_fts.shape
      for i, j, k in itertools.product(range(shape[0]), range(shape[2]), range(shape[3])):
        descriptor = np.concatenate([rgb_fts[i, :, j, k], flow_fts[i, :, j, k]])
        rs.addData(descriptor)
      del fts
