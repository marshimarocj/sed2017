...
def prepare_cfg():
  ...
  trn_neg_lst_file = os.path.join(root_dir, 'meta', 'trn_neg.25.c3d.tfrecords.lst')
  ...
  out_prefix = os.path.join(out_dir, 'netvlad.c3d.l2norm_input.0.%s.%d.%d'%(
    '_'.join([str(d) for d in track_lens]), num_center, neg2pos_ratio))
  ...
  proto_cfg['trn_neg2pos_in_batch'] = neg2pos_ratio
...
def prepare_focalloss_cfg():
  ...
  out_prefix = os.path.join(out_dir, 'netvlad.l2norm_input.focalloss.0.%s.%d.%d.nobalance'%(
    '_'.join([str(d) for d in track_lens]), num_center, gamma))
  if not os.path.exists(out_prefix):
    os.mkdir(out_prefix)
...
def gen_neg_lst_for_trn():
  ...
  ft_dir = os.path.join(root_dir, 'c3d', 'track_group_trn_split')
  # out_file = os.path.join(root_dir, 'meta', 'trn_neg.lst')
  # out_file = os.path.join(root_dir, 'meta', 'trn_neg.25.tfrecords.lst')
  out_file = os.path.join(root_dir, 'meta', 'trn_neg.25.c3d.tfrecords.lst')
