...
class ModelFocalLossCfg(ModelCfg):
  def __init__(self):
    ...
    self.alphas = [float(neg2pos)] + \
      [1./(self.num_class-1) for _ in range(self.num_class-1)]
  ...
  def add_loss(self, basegraph):
    with basegraph.as_default():
      with tf.variable_scope(self.name_scope):
        ...
        loss_op = - tf.to_float(self._labels) * tf.pow(1-p, self._config.gamma) * log_p # (None, num_class)
